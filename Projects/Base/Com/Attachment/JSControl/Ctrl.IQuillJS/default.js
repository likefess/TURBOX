...
function setQuillMutationObserver() {
...
function resizeQuill() {
    ...
    if (iframe) {
        iframe.height = vHeight + 10 + vHeight * 3 / 100;
    }
    else {
       window.callProcEx('ResizeBrowser', [Math.ceil(vHeight + vHeight * 7 / 100)]);
    }       
}

function setResizeEnabled(aResizeEnabled) {
    resizeEnabled=aResizeEnabled;
    if (resizeEnabled) { 
...
let hiddenControls = [];
const defaultHideControls = ['image', 'video', 'videoupload', 'table', 'link']

function hideToolbarControls(controls = defaultHideControls) {
    const toolbar = quill.getModule('toolbar')
    if (hiddenControls.length) {
        showToolbarControls(hiddenControls)
    }
    hiddenControls = []
    toolbar.controls.forEach(([name, node]) => {
        if (controls.includes(name)) {
            node.style.display = 'none'
            hiddenControls.push(name)
        }
    })
}

function showToolbarControls(controls) {
    const toolbar = quill.getModule('toolbar')
    toolbar.controls.forEach(([name, node]) => {
        if (controls.includes(name)) {
